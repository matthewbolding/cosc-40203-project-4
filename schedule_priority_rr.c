...
void schedule() {
    ...
    while((currentTask = pickNextTask()) != NULL) {
        ...
        if(num_top_pri == 1 || currentTask->burst < QUANTUM) {
            burst = currentTask->burst;
        } else {
            burst = QUANTUM;
        }
...
/**
 * @brief Function to return the task with highest priority in round robin fashion
 * 
 * @return Task* the task with highest priority in round robin fashion
 */
Task* pickNextTask() {
    if(head == NULL) return NULL;

    struct node *current = head;
    // Note head node's task as highest priority initially.
    Task *pri_task = head->task;
    num_top_pri = 1;
    
    // Interate, and change current, if a higher priority job is found.
    while(current->next != NULL) {
        current = current->next;
        if(current->task->priority > pri_task->priority) {
            pri_task = current->task;
            num_top_pri = 1;
        } else if(current->task->priority == pri_task->priority) {
            pri_task = current->task;
            num_top_pri++;
        }
    }
